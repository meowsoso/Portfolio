...
class Item {
  ...
  layout() {
    ...
    // translate the deco
    TweenMax.set(this.DOM.deco, {
      y: this.renderedStyles.decoTranslationY.previous
    });
  }
}
...
class SmoothScroll {
    ...
    this.renderedStyles = {
      ...
      skew: {
        ...
        setValue: () => {
          ...
          const val = Math.max(
            Math.min(
              MathUtils.map(scrollingSpeed, 20, 100, fromValue, toValue),
              toValue
            ),
            fromValue
          );
          return this.renderedStyles.translationY.previous < docScroll
            ? val
            : -1 * val;
        }
      }
  ...
  layout() {
    this.DOM.scrollable.style.transform = `translate3d(0,${-1 *
      ...
      this.renderedStyles.skew.previous
    }deg)`;
  }
  setSize() {
  ...
  initEvents() {
    ...
    $("a.nav").on("click", function(event) {
      ...
      if (this.hash !== "") {
        ...
        // Store hash
        var hash = this.hash;
        let scrollPosition = $(hash).offset().top;
